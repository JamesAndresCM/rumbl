defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    test "list_videos/0 returns all videos" do
      ...
      videos = Multimedia.list_videos() |> Repo.preload(:user)
      assert videos == [video]
    end
    ...
    test "get_video!/1 returns the video with given id" do
      user = user_fixture()
      video = video_fixture(user)
      assert Multimedia.get_video!(video.id) |> Repo.preload(:user) == video
    end
    ...
    test "update_video/2 with invalid data returns error changeset" do
      ...
      assert video == Multimedia.get_video!(video.id) |> Repo.preload(:user)
    end
